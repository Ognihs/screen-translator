...
class TestTranslationWorker:
    ...
    def test_worker_cancel(self):
        ...
        with patch("control_window.QThread.__init__"):
            ...
            worker = TranslationWorker(
                image_data=b"test_image_data", source_lang="Japanese", target_lang="Chinese", model="gpt-4o"
            )
    ...
    def test_worker_cancel_flag_prevents_result(self):
        """测试取消标志阻止结果发送"""
        with patch("control_window.QThread.__init__"), patch("control_window.translate_image") as mock_translate:
            from control_window import TranslationWorker
...
class TestStabilityChecker:
    ...
    def test_stability_checker_reset(self):
        ...
        checker = StabilityChecker(window_size=5, threshold=50.0, change_threshold=0.01)
